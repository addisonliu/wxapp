...
Page({
  ...
  getlistDatat() {
    ...
    util.request(api.bankApplyData, {empno: app.globalData.userInfo, site: app.globalData.site}).then(function (res) {
      if (res.result == "successful" && res.code == 1) {
        ...
        wx.showModal({
          content: '您已存在发薪卡号可点击查询卡号查看，如需变更，请选择第二项【银行卡号变更】',
  ...
  chooseType: function (e) {
    ...
    this.setData({
      index: index,
      bankcardtype: that.data.bankType[index].bankKey,
      bankName: that.data.bankType[index].bankName
    })
  },
