// packageHr/pages/eleven/eleven.js
...
Page({
  ...
  bindViewTap: function () {
    wx.navigateTo({
      url: '/packageHr/pages/dlelevenstatus/elevenstatus',
    })
  },
  ...
  getBaseData() {
    let that = this;
    util.request(api.eleven, {EmpNo: app.globalData.userInfo, Site: app.globalData.site,}).then(function (res) {
  ...
  var reg = /^1[3|4|5|7|8]\d{9}$/;   /*定义验证手机号表达式*/
   ...
   console.log(info)
    if (info.SkillName == '') {
      ...
      util.requests(api.elevensend , info ,"POST").then(function (res){
        console.log(res);
      if(res.result == "success"){
        console.log(info)
        wx.navigateTo({
          url: '/packageHr/pages/dlelevensuccess/index',
        })
   }else{ 
          wx.showLoading({
            title: res.result,
