...
const { ExecutionInit, ExecutionHistory,
  ExecutionSheet, ExecutionAddWorker, ExecutionSubmit,
  IntervieweeInfo } = require('../../../config/api.js');
...
Page({
  ...
  data: {
    ...
    showAddDialog: false,
    pageList1: [//页面1
      { label: '施工厂商负责人手机号', isRequired: true, id: 'GuestMobile', disabled: false, type: 'input' },
      ...
      { label: '受访同仁英文名', isRequired: true, id: 'EnName', disabled: false, type: 'input', otherType: 'picker', otherLabel: '点击选择' },
    ...
    pageList2: [
      { label: '申请作业类别', isRequired: false, id: 'applytype', disabled: true, type: 'input' },
      ...
      { label: '施工开始日期', isRequired: true, id: 'fromdate', disabled: true, type: 'input', otherLabel: '点击选择', start: new Date().toLocaleString().split(' ')[0].replace(/\//g, '-') },
      { label: '施工结束日期', isRequired: true, id: 'todate', disabled: true, type: 'input', otherLabel: '点击选择', start: new Date().toLocaleString().split(' ')[0].replace(/\//g, '-') },
      {label: '对水电需求', isRequired: true, id: 'require', disabled: false, type: 'radio', radioList: [
        { label: '有需求', value: 'Y' }, { label: '无需求', value: 'N'}
      ]},
      { label: '具体水电需求', isRequired: false, id: 'requireqqq', disabled: false, type: 'input' },
      {
        label: '施工类型', isRequired: true, id: 'isworthy', disabled: false, type: 'radio',
        radioList: [
    ...
    http({ url: ExecutionInit }).then(res => {
      if (res.data.code === 1) {
        this.setData({
          ...
          applytype: res.data.data.applytype[0],
          data: {
            ...this.data.data,
            applytype: res.data.data.applytype[0].Name
          }
        })
      }
      else {
    ...
    http({ url: ExecutionHistory, data: { telephone } }).then(res => {
      if (res.data.code === 1) {
        ...
        if (Detail.length) {
          Detail.forEach((v, k) => {
            arr.push({
              GuestName: v.GuestName, AppIDNO: v.AppIDNO, key: k,
              checked: false,
  ...
  //显示增加施工人员dialog
  showAddWorkerDialog: function () {
    this.setData({
      showAddDialog: true,
      addWorkerDialogAnimation: 'dialog-toShow'
    })
  },
  hideAddWorkerDialog: function () {
    this.setData({
